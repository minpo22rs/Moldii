...
use App\Models\Tb_otp_changephone;
use App\Models\User;
use Carbon\Carbon;
use Session;

class OtpChangephoneController extends Controller
{
    ...
    public function create($number)
    {
        ...
        $sender = 'Maemod';
        $msisdn = $phone;
        $msg = 'รหัสยืนยันการเปลี่ยนเบอร์โทรศัพท์ ' . $otp;
    ...
    public function check($otp,$phone)
    {

        $tel = $phone;
        $otp = $otp;
        
        $tel_otp = Tb_otp_changephone::where(['otp_code' => $otp,'tel'=>$tel])->first();
       
        if ($tel_otp != null) {
            $id =$tel_otp->otp_id;
            Tb_otp_changephone::where('otp_changephone_id', $id)
