    ...
    $(document).on('change', '.time', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (len > 0) {
                    for(var i=0; i < len; i++){
                        var id = response['data'][i].event_sp_id;
                        var name = response['data'][i].event_sp_product_id;
                        var option = "<option value='"+id+"' selected>"+name+"</option>"; 
                        $('#product_'+i+' option').filter(':selected').removeAttr('selected');  
                        // $('#product_'+i).val(4).attr("selected", true);
                        $('#product_'+i).val(4).find("option[value=" + 4 +"]").prop('selected');
                    }
                }
            }
        });
